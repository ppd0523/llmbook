...
- 독자가 본문의 지시대로 입력하는 경로는 배치 규칙보다 우선한다. 규칙을 맞추려고 독자의 실행 절차를 바꾸지 않는다.
- 전체 책 목록은 `docs/index.md`, 책 목차는 해당 책의 `index.md`에서 관리한다. `mkdocs.yml`에 수동 `nav` 목록을 추가하지 않는다.
- 탐색 메뉴의 책 이름과 순서는 `docs/<book-slug>/.pages`와 `docs/.pages`에서 관리한다. 책을 추가·삭제·개명하면 `docs/index.md`와 이 두 곳을 함께 갱신한다.
