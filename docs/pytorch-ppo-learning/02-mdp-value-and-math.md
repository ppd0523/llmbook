...
**Advantage(이점) 함수**는 특정 행동 가치가 상태의 평균 가치보다 얼마나 나은지를 나타낸다.

$$
A^\pi(s,a)=Q^\pi(s,a)-V^\pi(s)
$$

양수면 그 상태의 정책 평균보다 좋은 행동, 음수면 나쁜 행동이다. PPO actor가 직접 사용하는 핵심 신호다.
...
PPO에서는 새 정책과 old 정책의 같은 행동 확률 비율을 계산한다.
...
예를 들어 old 확률이 0.4, 새 확률이 0.5라면 ratio는 $0.5/0.4=1.25$다. 같은 계산을 로그확률 차이의 지수로 수행하면 수치적으로 안정적이다.
...
학습으로 크게 만들고 싶은 기준을 **목적함수(objective)** $J(\theta)$라 한다. 반대로 optimizer가 작게 만들 값은 **손실(loss)** 이다. 기대 return을 최대화하고 싶다면 loss를 $-J(\theta)$로 만들어 최소화할 수 있다. 부호가 바뀌었을 뿐 같은 목표다.
...
확률정책의 기댓값을 직접 미분하기 어려울 때 다음 항등식을 쓴다.

$$
\nabla_\theta p_\theta(x)=p_\theta(x)\nabla_\theta\log p_\theta(x)
$$

이 결과로 policy gradient를 샘플 평균 형태로 추정할 수 있다.
...
$\hat A_t>0$이면 선택 행동의 로그확률을 올리고, $\hat A_t<0$이면 내리는 방향이다. 환경 전이를 미분하는 항은 없다.

## Advantage 표준화

Policy gradient 추정의 분산이 크면 update 방향이 batch마다 심하게 흔들린다. Critic의 가치처럼 비교 기준으로 빼는 **baseline**은 이 분산을 줄이는 도구다. 다음 장에서 평균 gradient를 바꾸지 않는 이유를 계산한다.

구현에서는 advantage를 자주 표준화한다.

$$
\hat A'_t=\frac{\hat A_t-\operatorname{mean}(\hat A)}{\operatorname{std}(\hat A)+10^{-8}}
$$

이는 PPO의 유일한 정의가 아니라 널리 쓰이는 안정화 선택이다. 부호와 상대적 크기는 유지하면서 optimizer가 다루는 scale을 정돈한다.
...
??? note "정답 확인"
    1번: $G_2=3$, $G_1=0.5$, $G_0=2.25$. 2번: $0.8$. 3번: 자연 종료이므로 다음 가치를 버려 $1-0.4=0.6$. 4번: 시간 제한은 bootstrap하므로 $1+0.9(100)-0.4=90.6$. 5번: 낮추는 방향이다.
