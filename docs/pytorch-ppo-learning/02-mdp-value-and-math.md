...
!!! warning "잔차를 쓴다고 모두 잔차 학습은 아니다"
    PPO가 TD 잔차로 advantage를 계산한다는 사실만으로 PPO 전체를 “잔차 강화학습”이라 부르지는 않는다. “잔차”가 가리키는 네 대상은 4장에서 표로 구분하며, 그중 잔차 강화학습은 기존 제어 행동에 학습된 보정 행동을 더하는 별도 설계여서 8장에서 따로 다룬다. ResNet의 잔차 연결도 또 다른 신경망 구조 용어다.
