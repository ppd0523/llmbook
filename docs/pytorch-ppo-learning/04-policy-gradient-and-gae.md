...
6. 회귀 잔차, TD 잔차, 신경망 잔차 연결, 잔차 강화학습을 구분한다.
...
## “잔차”가 가리키는 네 가지를 구분하기

**잔차 학습(residual learning)** 은 완성된 출력을 처음부터 모두 학습하는 대신, 이미 있는 예측·입력·제어값에서 부족한 **차이 또는 보정값**을 학습한다는 넓은 생각이다. 하지만 강화학습 자료에서 “잔차”는 서로 다른 네 대상을 가리키므로, 수식의 출력이 무엇인지를 그때마다 확인해야 한다. 바로 앞에서 본 $\delta_t$는 그중 두 번째다.

| 문맥 | 잔차가 뜻하는 것 | 이 책과의 관계 |
|---|---|---|
| 회귀 잔차 | target과 예측의 차이 $y-\hat y$ | Critic loss와 explained variance에 사용 (2·7장) |
| TD 잔차 | 한 스텝 TD target과 가치 예측의 차이 $\delta_t$ | GAE가 합산하는 advantage 신호 (이 장) |
| 신경망 잔차 연결 | 층 입력에 학습 보정 $F(h)$를 더한 $h+F(h)$ | ResNet 계열 구조이며 PPO의 필수 조건은 아님 |
| 잔차 강화학습 | 기존 제어 행동에 학습한 보정 행동을 더함 | 기존 제어기가 있는 로봇 제어의 확장 방법 (8장) |

네 번째인 잔차 강화학습은 정책의 출력 자체를 바꾸는 별도 설계이므로 연속 행동을 다루는 8장에서 따로 본다.
