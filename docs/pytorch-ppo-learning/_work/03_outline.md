...
## 12. 3차 개정: 장 경계 재설계 (2026-09-22)

2장이 "PPO를 위한 최소 수학"이라는 제목과 달리 4·5·6장의 알고리즘 수식을 전방 참조 없이
선취해, 독자가 같은 수식을 두 번 다른 깊이로 만나면서 왜 반복되는지 알 수 없었다.
장 경계를 **수학 도구 ↔ 알고리즘**으로 다시 그었다.

| 내용 | 이전 위치 | 새 위치 | 근거 |
|---|---|---|---|
| probability ratio 유도와 수치 예 | 2장 로그 절 | 5장 | ratio는 PPO 목적함수의 구성요소다 |
| 로그미분 트릭의 policy gradient 추정식 | 2장 | 4장 (네 단계 유도) | 4장이 이미 같은 유도를 더 자세히 갖고 있었다 |
| advantage 표준화 수식 | 2장 | 6장 (코드 옆) | 수식과 tensor를 붙여 보는 것이 이 책의 축이다 |
| advantage의 추정 방법 | 2장 | 4장 (baseline·GAE) | 2장에는 $A^\pi=Q^\pi-V^\pi$ 정의만 남긴다 |
| "잔차" 네 문맥 구분 표 | 8장 | 4장 (TD 잔차 직후) | 혼동이 생기는 자리에서 답해야 한다 |
| 후속 학습 키워드 목록 | 2장 + 9장 | 9장 | 2장은 용어 뜻과 참조만 |
| ablation 후보 표 | 7장 + 9장 | 7장 | 7장=방법론, 9장=수행 절차와 rubric |

2장은 343줄에서 약 280줄로 줄었고, 4·5·6장은 각자 이미 갖고 있던 설명에 흡수했으므로
분량 증가가 거의 없다.

**하지 않은 재구성:** 6장(444줄, 21개 절)의 분할을 검토했으나 하지 않았다. 절단면 후보는
`## 프로그램의 다섯 단계` 앞과 `## KL 조기 중단` 앞이었다. 6장은 "하나의 프로그램을 처음부터
끝까지 만든다"는 단일 서사이고 그것이 이 책의 중심 약속이다. 특히 두 번째 절단면은
"평가·체크포인트는 별도 주제"라는 잘못된 신호를 준다. index가 이미 8–12시간으로 최장임을
알리고 있어 길이는 예고된 비용이다.

파일 분할·병합이 없었으므로 공개 URL은 하나도 바뀌지 않았다.
