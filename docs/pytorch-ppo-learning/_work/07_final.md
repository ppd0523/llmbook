...
## 3차 개정 표기 정리 (2026-09-22)

반복되던 제목·형식 불일치를 통일했다.

| 항목 | 이전 상태 | 통일 후 |
|---|---|---|
| 오류 절 제목 | `흔한 오해`(1장), `흔한 오류`(3·4·5장), `흔한 실행 문제`(6장) | `## 흔한 오류`. 1장 표의 열 이름은 `흔한 오해`로 명시 |
| 참고문헌 제목 | `대학 강의와 추가 읽기`, `대학 강의와 참고문헌`, `참고문헌`, `추가 읽기`, `핵심 참고자료` 5종 | `## 참고문헌` |
| Worked Example 헤딩 | `##`(1·4·5장)과 `###`(2·3장) 혼용 | `##` |
| 논문 인용 형식 | `et al.`(4·5·6·8장)과 `외`(9장) 혼용, 9장은 연도 누락 | `저자 et al. (연도), [*제목*](url)` |

절 제목이 없는 장(2·7·8·9장의 오류 절)에 새 절을 만들지는 않았다. 없는 내용을 지어내는 것은
퇴고가 아니라 집필이다.

`## 정리`를 새로 넣었으므로 각 장의 순서는 본문 → `## 흔한 오류`(있는 장) → `## 정리` →
`## 연습문제` → `## 참고문헌` → 푸터 내비게이션이다. 9장은 연습문제 대신
`## 최종 개념 점검`을 쓰며 그 앞에 `## 정리`가 온다.
