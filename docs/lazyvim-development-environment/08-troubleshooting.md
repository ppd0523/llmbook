...
문제가 생긴 뒤 처음으로 상태를 보면 무엇이 달라졌는지 알기 어렵다. 다섯 언어 smoke test가
성공한 시점에 다음 정보를 기록한다.
...
Rust, C++ smoke test를 다시 수행한다. 문제가 없다면 diff와 함께 commit한다.
...
| C++ build | `cmake --build build` | compiler, include, link, executable target |
...
6. Client가 선택한 root가 언어 manifest나 C++ compile database가 있는 project인가?
...
Rust는 Mason보다 rustup component와 shell PATH를 먼저 확인한다. C++에서 clangd가
attach되었는데 header나 compile option diagnostic이 틀리면 `build/compile_commands.json`과
실제 CMake build command를 비교한다.

## 8.7 Formatting이 동작하지 않을 때
...
6. C++는 clangd가 attach되고 `.clang-format`을 찾는가?
7. `<leader>uF` 또는 `<leader>uf`로 autoformat을 꺼 두지 않았는가?
8. 같은 filetype에 여러 formatter가 순차 실행되지 않는가?
...
| C++ | `codelldb` | CMake Debug build, selected executable target |

DAP UI만 열리고 program이 실행되지 않으면 UI layout을 수정하지 않는다. Adapter와
configuration부터 확인한다.

## 8.9 WSL에서만 생길 수 있는 문제

### Windows와 Linux binary 혼용

```console
$ command -v nvim git node npm python3 cargo c++ cmake clangd
$ file "$(command -v nvim)"
```
...
- [ ] clangd가 C++ buffer에 attach되고 `compile_commands.json`을 읽는다.
- [ ] Source/header 전환과 project include navigation이 동작한다.
- [ ] `<leader>cf`가 `.clang-format` policy를 적용한다.
- [ ] CMake Debug target을 build하고 CodeLLDB breakpoint에 정지한다.
...
6. C++ project는 build되지만 clangd가 project header를 찾지 못한다. 어떤 두 출력을 비교해야 하는가?
...
- [clangd project 설정](https://clangd.llvm.org/installation)
