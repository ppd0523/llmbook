...
LazyVim을 설치하고 JavaScript, TypeScript, Python, Rust, C++의 Language Server
...
내용은 2026-08-19의 공식 LazyVim 문서와 Starter를 기준으로 검토했다. 이 날짜의
...
- vtsls, Pyright, Ruff, rust-analyzer, clangd가 buffer에 연결되었는지 확인한다.
- Prettier, Ruff, rustfmt, clang-format으로 format source를 선택하고 충돌을 피한다.
...
6. [Rust](./06-rust.md)
7. [C++](./07-cpp.md)
8. [운영과 문제 해결](./08-troubleshooting.md)
...
| C++ | clangd, clangd_extensions.nvim | clang-format | CodeLLDB |

Mason은 편집기에서 사용하는 외부 도구를 간편하게 설치한다. 반면 compiler와 application
runtime, library, project lockfile, `package.json`, `pyproject.toml`, `Cargo.toml`,
`CMakeLists.txt`는 각 프로젝트와 language toolchain이 소유한다. 이 경계를 유지해야
편집기 설정이 프로젝트의 build·dependency 관리를 대신하지 않는다.
