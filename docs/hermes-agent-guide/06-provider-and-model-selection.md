...
aggregator는 모델 전환과 fallback이 편하고, native provider는 인증과 데이터 경로가
단순할 수 있다. 이름만 보고 고르지 말고 실제 저장소, 언어, 도구 모음으로 시험한다.
local model은 parameter 수보다 충분한 context 길이와 구조화된 tool call 호환성을 먼저
확인한다. Hermes quickstart의 기준은 최소 64K-token context다.

## 호출 목적에 따라 model slot을 나눈다

모든 호출을 가장 비싼 main model 하나로 처리할 필요는 없다.
...
| fallback | main 경로가 실패했을 때 세션을 잇는 대체 경로 | 다른 제공자와 필요한 capability |
| session override | 현재 세션에서만 쓰는 모델 | 일회성 비교 또는 고난도 판단 |

장기 역할의 품질 차이는 프로필의 main model로 고정한다. 같은 프로필의 반복적인
부가 작업은 auxiliary slot으로, fresh-context 하위 작업은 delegation model로 분리한다.

## 작업 성격으로 model tier를 고른다
...
## 작은 acceptance suite로 비교한다

자신의 반복 업무에서 대표 작업 5~10개를 고른다. 각 후보를 같은 입력과 도구로 실행해
다음 항목을 기록한다.
...
| profile | main model | auxiliary | 이유 |
|---|---|---|---|
...
## `/model`은 현재 session의 예외에 쓴다

Discord에서 `/model`을 보내면 이미 인증된 provider와 model을 picker로 고를 수 있다.
현재 세션에는 즉시 적용되지만 모델이 바뀌면 prompt cache가 reset되어 다음 메시지가
전체 대화 입력을 다시 처리할 수 있다. 새 모델의 context 한계에 가까우면 다음 메시지
전에 compression이 실행될 수도 있다.
...
## fallback은 기능을 보존하는 다른 경로로 둔다

fallback은 동일 provider의 같은 장애를 반복하지 않도록 다른 provider를 섞는다. main
model이 tool use, vision, 긴 context를 필요로 한다면 fallback도 필요한 capability를
