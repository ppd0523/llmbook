...
이 선언은 저장소의 [Neovim 설정](../assets/example-config/dotfiles/nvim/init.lua)을 `~/.config/nvim`에 연결한다. Home Manager가 관리하는 대상 파일을 홈 디렉터리에서 직접 고치면 다음 전환 때 사라지거나 충돌한다.
