...
Home Manager owns the generated `~/.config/nvim/init.lua`. The configuration
therefore does not link the whole `~/.config/nvim` directory: it links the
Git-managed `lua/` subtree, `stylua.toml`, and the writable base
`lazy-lock.json` separately. Linking the parent directory would conflict with
the `init.lua` generated by `programs.neovim` and make the Home Manager build
fail with an `outside $HOME` error.
