...
작성 호스트에는 Nix, NixOS-WSL, Neovim, Lua 실행기가 없다. 따라서 다음은 실제
NixOS-WSL 또는 네이티브 NixOS에서 추가로 확인해야 한다.

- `nix flake check`, `nixos-rebuild build`, standalone Home Manager `build`
...
- 예제는 `~/.config/nixos`를 고정 clone 위치로 사용한다. 다른 위치를 선택하면
  `mkOutOfStoreSymlink` 대상도 바꿔야 한다.
...
- Nix 평가와 Neovim 실행 검증은 실제 NixOS 환경의 후속 검증 항목으로 명시했다.
