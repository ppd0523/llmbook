...
이 책은 고등학교 수준의 행렬과 삼각함수를 아는 학습자가 **역기구학(Inverse Kinematics, IK)** 을 처음 배우기 위한 자료다. 제어기, 모터, 동역학은 다루지 않는다. 대신 관절각과 손·발의 목표 위치·방위를 연결하는 기구학을 차근차근 익힌다.
...
| 4 | 자코비안(Jacobian)과 수치 IK | 관절의 작은 변화와 손의 작은 변화를 연결하는 행렬로 오차를 반복해서 줄인다. | 10–14시간 |
