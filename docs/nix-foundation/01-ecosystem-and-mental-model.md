...
평가가 요구한 결과가 Store에 없으면 미리 빌드한 결과 저장소인 binary cache에서
내려받거나 직접 빌드한다. Nix 문서에서 “build”는 이 두 경우를 모두 포함해 Store
결과를 확보한다는 뜻으로 자주 쓰인다.

### 활성화

만든 결과를 현재 사용 상태로 전환한다.

이때 profile은 선택한 Store 결과를 노출하는 링크이고, generation은 profile의
번호가 매겨진 한 상태다. 자세한 구조는 3장에서 설명한다.
...
필요한 이유다. Nix source와 lockfile을 함께 보관해야 다른 사람이 같은 입력을
선택할 수 있다.
...
이 규칙은 같은 Store path가 모든 컴퓨터에서 언제나 만들어진다는 약속과는 다르다.
지원하는 시스템, 외부 binary cache, 비밀 값처럼 구성 밖에서 달라지는 조건은 별도로
관리해야 한다. 입문 단계에서는 먼저 **입력과 적용 범위를 기록한다**는 원칙을 지킨다.
