...
1. `nix shell`, `nix develop`, `nix-shell`의 용도를 구분한다.
2. `nixpkgs#hello` 형태의 installable을 읽는다.
3. 임시 도구와 영구 package를 올바른 계층에 둔다.
...
package의 실행 파일이 필요할 때 쓴다. package 빌드 환경 전체나 프로젝트 환경 변수
설계가 목적은 아니다.
...
현재 Flake의 `devShells.<system>.default`를 먼저 찾는다. 프로젝트가 선언한 compiler,
library, 환경 변수, `shellHook`을 제공한다. `--command`로 CI와 비대화형 실행에도
사용할 수 있다.

```console
$ nix develop --command bash -c 'tool --version && run-tests'
```
...
오래된 튜토리얼에서 많이 보인다. `shell.nix`나 `default.nix` 기반 프로젝트를
사용하거나 legacy shebang을 유지해야 할 때 필요하다. 새 Flake 프로젝트의 기본
학습 흐름에서는 `nix develop`을 사용한다.
...
## 4.4 `nix run`, `nix build`, `nix profile`과의 차이
...
`nix run`은 app 또는 실행 가능한 package를 바로 실행하므로 별도 shell이 필요 없다.
...
package 속성 이름과 프로그램 실행 파일 이름이 항상 같지는 않다. 검색 결과와
[NixOS package 검색](https://search.nixos.org/packages)을 함께 확인한다.
...
- `nix develop`은 프로젝트가 선언한 개발 또는 빌드 환경에 들어간다.
